...
class Scheduler:
    ...
    def can_add_page(self, obj_url: ParseResult, depth: int) -> bool:
        ...
        if depth > self.depth_limit:
           #print(RED + "[FALHA] " + CYAN + obj_url.geturl() + " está alem do limite de profundidade!" + RESET)
            return False
        elif obj_url.geturl() in self.set_discovered_urls:
           #print(f"{RED}[FALHA] {obj_url.geturl()}{CYAN} Pagina repetida! {RESET}")
            return False
        else:
            print(CYAN + "A url: " + obj_url.geturl() +  " pode ser adicionada!" + RESET)
            return True
            ...
            while self.dic_url_per_domain:
                for key in self.dic_url_per_domain:
                    if key.is_accessible():  # Encontra o primeiro servidor acessivel
                        ...
                        #print(f"{GREEN}[SUCESSO] {CYAN}Servidor: {key} acessado!" + RESET)

                        if len(self.dic_url_per_domain[key]) <= 0:
                            self.dic_url_per_domain.pop(key)
                            print(f"{GREEN}[SUCESSO] {CYAN}Servidor: {key} removido! {RESET}")
